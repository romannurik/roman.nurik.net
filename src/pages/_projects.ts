...
function setupFiltering() {
  ...
  let applyFilter = (filter: string, scrollTop = true) => {
    ...
    for (let project of document.querySelectorAll('section.project')) {
      ...
      if (wasHidden) {
        ...
        setTimeout(() => (project.querySelector('rn-carousel') as any).snapToPage(0, { immediate: true }));
      }
    }
  ...
  for (let tab of document.querySelectorAll('.filter input')) {
    tab.addEventListener('click', ev => {
      let filter = (ev.currentTarget as HTMLInputElement).value;
      applyFilter(filter);
...
function setupVideoMedia() {
  ...
  let playPauseVideoMedia = (media: any, play: boolean) => {
  ...
  let observer = new IntersectionObserver(entries => {
    for (let entry of entries) {
      (entry.target as any).visible = entry.intersectionRatio >= 0.5;
...
function setupMediaSizing() {
  ...
  let sizeMedia = (media: any) => {
    ...
    let cropPosition: any = computedStyle.getPropertyValue('--crop-position') || null;
    if (cropPosition) {
      // assume percentage
  ...
  for (let video of document.querySelectorAll('.media video')) {
    video.addEventListener('resize', ev => {
      sizeMedia((ev.currentTarget as any).closest('.media'));
    });
  }
